...
class Exercise2 {
    ...
    void personHasNotEmptyLastNameAndFirstName() {
        ...
        Predicate<Person> validate = person -> person != null &&
    ...
    void personHasNotEmptyLastNameAndFirstNameUsingLogicalOperators() {
        Predicate<Person> personHasEmptyFirstName = person -> person == null || person.getFirstName().isEmpty();
        Predicate<Person> personHasEmptyLastName = person -> person == null || person.getLastName().isEmpty();
    ...
    private <T> Predicate<T> and(Predicate<T> left, Predicate<T> right) {
        return param -> left.test(param) && right.test(param);
    }
